...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    if((pte = walk(pagetable, a, 0)) == 0)
      continue; // Skip missing page tables
    if((*pte & PTE_V) == 0)
      continue; // Skip invalid mappings
    if(PTE_FLAGS(*pte) == PTE_V)
      panic("uvmunmap: not a leaf");
    if(do_free){
      // Skip shared pages (PTE_S) - they're managed by SHM system
      // They will be freed via shmdt when all processes detach
      if(!(*pte & PTE_S)) {
...
int
copyout(pagetable_t pagetable, uint64 dstva, void *src, uint64 len)
{
  uint64 n, va0, pa0;
  pte_t *pte;
